...
group("select * from customer where id = ?", () => {
  ...
  const prepared = drizzlePrepared
    .select()
    .from(customers)
    .where(eq(customers.id, sql.placeholder("userId")))
...
group("select * from customer where company_name ilike ?", () => {
  ...
  const prepared = drizzlePrepared
    .select()
    .from(customers)
    .where(sql`${customers.companyName} ilike ${sql.placeholder("name")}`)
...
group('"SELECT * FROM employee"', () => {
  ...
  bench("postgresjs", async () => {
    await pgjs`select * from "employees"`;
  });
...
group("select * from employee where id = ? left join reportee", () => {
  ...
  bench("postgresjs", async () => {
    for (const id of employeeIds) {
      ...
      left join "employees" as "e2" on "e2"."id" = "e1"."recipient_id"
      where "e1"."id" = ${id}
    `;
    }
  });
  ...
  const prepared = drizzlePrepared
    ...
    .from(employees)
    .leftJoin(e2, eq(e2.id, employees.recipientId))
    .where(eq(employees.id, sql.placeholder("employeeId")))
...
group("SELECT * FROM supplier", () => {
  ...
  bench("postgresjs", async () => {
    await pgjs`select * from "suppliers"`;
  });
...
group("select * from supplier where id = ?", () => {
  ...
  const prepared = drizzlePrepared
    .select()
    .from(suppliers)
    .where(eq(suppliers.id, sql.placeholder("supplierId")))
...
group("SELECT * FROM product", () => {
  ...
  bench("postgresjs", async () => {
    await pgjs`select * from "products"`;
  });
...
group("SELECT * FROM product LEFT JOIN supplier WHERE product.id = ?", () => {
  ...
  const prepared = drizzlePrepared
    ...
    .from(products)
    .leftJoin(suppliers, eq(products.supplierId, suppliers.id))
    .where(eq(products.id, sql.placeholder("productId")))
...
group("SELECT * FROM product WHERE product.name ILIKE ?", () => {
  ...
  const prepared = drizzlePrepared
    .select()
    .from(products)
    .where(sql`${products.name} ilike ${sql.placeholder("name")}`)
